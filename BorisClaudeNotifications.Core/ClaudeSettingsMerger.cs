...
{
    private const string HooksExeName = "BorisClaudeNotifications.Hooks.exe";
    ...
    private static void MergeStatusLine(JsonObject root, string hooksExePath, List<string> warnings)
    {
        ...
        if (root["statusLine"] is JsonObject existing)
        {
            ...
            {
                warnings.Add(
                    "An existing statusLine command was left in place; BorisClaudeNotifications will not receive " +
    ...
    private static void WriteAtomically(string path, JsonObject root, bool backup)
    {
        ...
        if (backup)
        {
            ...
            var backupPath = path + ".borisclaudenotifications.bak";
