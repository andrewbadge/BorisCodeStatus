...
internal static class FirewallGuard
{
    ...
    private const string RuleDisplayName = "BorisCodeStatus";
    ...
    private static string TrayExecutablePath =>
        Environment.ProcessPath ?? Path.Combine(AppContext.BaseDirectory, "BorisCodeStatus.Tray.exe");
    ...
    public static void ShowFirstRunNoticeIfNeeded(int port = DefaultPort)
    {
        try
        {
            ...
            MessageBox.Show(
                "Windows is about to ask whether to allow BorisCodeStatus through the firewall.\r\n\r\n" +
                ...
                "BorisCodeStatus — firewall prompt incoming",
    ...
    public static string BuildRemediationScript(bool includePublic)
    {
        ...
        script.AppendLine("# BorisCodeStatus - allow the ESP32 display to reach the status endpoint.");
        ...
        script.AppendLine("  Where-Object { $_.DisplayName -like '*BorisCodeStatus*' } |");
        ...
        script.AppendLine();
        script.AppendLine("Write-Host 'BorisCodeStatus: firewall rule applied.' -ForegroundColor Green");
