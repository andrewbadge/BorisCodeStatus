...
namespace BorisClaudeNotifications.Hooks;
...
/// (<c>BorisClaudeNotifications.Hooks.exe statusline</c>, <c>... notification</c>, and so on).
///
...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            Console.Error.WriteLine($"BorisClaudeNotifications: unrecognised verb '{verb}'.");
            return 0;
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"BorisClaudeNotifications: {ex.Message}");
            return 0;
        }
    }
