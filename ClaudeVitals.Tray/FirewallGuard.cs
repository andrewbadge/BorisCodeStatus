...
internal static class FirewallGuard
{
    ...
    private const string RuleDisplayName = "Claude Vitals Relay";
    ...
    private static string TrayExecutablePath =>
        Environment.ProcessPath ?? Path.Combine(AppContext.BaseDirectory, "ClaudeVitals.Tray.exe");
    ...
    public static void ShowFirstRunNoticeIfNeeded()
    {
        try
        {
            ...
            MessageBox.Show(
                "Windows is about to ask whether to allow Claude Code Vitals Relay through the firewall.\r\n\r\n" +
                ...
                "Claude Code Vitals Relay — firewall prompt incoming",
    ...
    public static string BuildRemediationScript(bool includePublic)
    {
        ...
        script.AppendLine("# Claude Code Vitals Relay - allow the ESP32 display to reach the status endpoint.");
        ...
        script.AppendLine("  Where-Object { $_.DisplayName -like '*ClaudeVitals*' -or $_.DisplayName -like '*Claude Vitals*' } |");
        ...
        script.AppendLine($"Get-NetFirewallRule -DisplayName '{RuleDisplayName}' -ErrorAction SilentlyContinue |");
        script.AppendLine("  Remove-NetFirewallRule -ErrorAction SilentlyContinue");
        script.AppendLine();
        ...
        script.AppendLine();
        script.AppendLine("Write-Host 'Claude Vitals Relay: firewall rule applied.' -ForegroundColor Green");
