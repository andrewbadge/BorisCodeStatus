...
internal static class FirewallGuard
{
    ...
    private const string RuleDisplayName = "BorisClaudeNotifications";
    ...
    private static string TrayExecutablePath =>
        Environment.ProcessPath ?? Path.Combine(AppContext.BaseDirectory, "BorisClaudeNotifications.Tray.exe");
    ...
    public static void ShowFirstRunNoticeIfNeeded(int port = DefaultPort)
    {
        try
        {
            ...
            MessageBox.Show(
                "Windows is about to ask whether to allow BorisClaudeNotifications through the firewall.\r\n\r\n" +
                ...
                "BorisClaudeNotifications — firewall prompt incoming",
    ...
    public static string BuildRemediationScript(bool includePublic)
    {
        ...
        script.AppendLine("# BorisClaudeNotifications - allow the ESP32 display to reach the status endpoint.");
        ...
        script.AppendLine("  Where-Object { $_.DisplayName -like '*BorisClaudeNotifications*' } |");
        ...
        script.AppendLine();
        script.AppendLine("Write-Host 'BorisClaudeNotifications: firewall rule applied.' -ForegroundColor Green");
