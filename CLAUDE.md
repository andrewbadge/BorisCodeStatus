...
```bash
dotnet build -c Release                       # also produces BorisCodeStatus.Installer\bin\Release\BorisCodeStatus.msi
dotnet build -c Release -p:Version=1.2.3      # versioned MSI
dotnet test BorisCodeStatus.Core.Tests
dotnet test BorisCodeStatus.Core.Tests --filter "FullyQualifiedName~SessionStatusTests"
dotnet test BorisCodeStatus.Core.Tests --filter "DisplayName~IdleTurnIsStillAnActiveSession"
```
...
Claude Code ──stdin JSON──▶ BorisCodeStatus.Hooks.exe ──writes──▶ %LOCALAPPDATA%\BorisCodeStatus\state.json
                            (runs once per event, exits)                    │ FileSystemWatcher
                                          BorisCodeStatus.Tray.exe ──hosts──▶ GET /status :5080 ◀── ESP32
```
...
network call to `BorisCodeStatus.Hooks`.
