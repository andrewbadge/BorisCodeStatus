...
{
    private const string MutexName = @"Local\FidoRelay.state.lock";
    ...
    /// <remarks>
    /// Carries over state from a pre-rename install before the first read, so an upgrade does not
    /// start from an empty display. Only on the shared instance — a store constructed with an
    /// explicit path (tests) has nothing to migrate.
    /// </remarks>
